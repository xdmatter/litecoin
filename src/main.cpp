...
#if defined(NDEBUG)
# error "Xdmatter cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xaea2efa9f57dc100b44b58cb7b1c27b663f4f03c8f143a0c100468cffa0df3e6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Xdmatter: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Xdmatter Signed Message:\n";
...
{
    // Xdmatter: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Xdmatter: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Xdmatter: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Xdmatter: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Xdmatter: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Xdmatter: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1e06c20eab5be69e44c735837315416d2db3e62008880e90403a796ffa2f0dec");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "THURSDAY, JANUARY 18, 2018. NOTHING HAPPENED.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043e94a600b3c9bbc94cc7940cbd6dab426111e2d5efd7455c9023583e7bf5dc4288ed0cfd4f30f44bae0268658a226563a9a32e12c9d142a7a48a3634862e1ff0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516304688;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084724417;

        if (fTestNet)
        {
            block.nTime    = 1516304681;
            block.nNonce   = 385282315;
        }

        // `true` For mining genesis block
        ...
        assert(block.hashMerkleRoot == uint256("0x929852362b081929c2eb025b7eb1fa3491172892770538a112dd0c32282f3f59"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Xdmatter: increase each by adding 3 to litecoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XdmatterMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XdmatterMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static XdmatterMiner(CWallet *pwallet)
{
    printf("XdmatterMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xdmatter-miner");
    ...
    try { loop {
        ...
        printf("Running XdmatterMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("XdmatterMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&XdmatterMiner, pwallet));
}
